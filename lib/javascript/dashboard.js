...
function tack_feed_to_dom_extension(id){
    $.ajax({
        type: "POST",
        dataType: 'json',
...
function tack_feed_to_dom() {
    ...
    $.ajax({
        ...
        success: function(response) {
            tack_loader(response, '#favorites');
        }
    });
...
function tack_loader(response, container) {
        ...
        success: function(response) {
            if(response.error == false) {
                for(i = 0; i < response.data.length; i++) {
                    ...
                    // tack_feed_to_dom_extension('#following_board_' + response.data[i].id);
                }
            }
        }
    });
