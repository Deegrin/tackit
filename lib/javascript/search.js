...
$(document).ready(function() {
    ...
    $(document).on('click', ".follow-board", function(e) {
        e.preventDefault();
    ...
    $(document).on('click', ".delete-tack", function() {
        var tack_id = $(this).data('tack');
        $.ajax({
            type: "POST",
            dataType:'json',
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.error == true) {
                    ...
                    alert("Retacked Tack to Dashboard");
                    location.reload();
                }
            }
        });       
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if($('#activeSearch').text() == "Tacks"){
                    ...
                    $('#searchResultType').html('<h3>Tack results for "' + search_param +'" (' + response.data.length +' results)</h3>');
                ...
                if($('#activeSearch').text() == "Boards"){
                    $('#search_results').empty();
                    $('#searchResultType').html('<h3>Board results for "' + search_param + '" (' + response.data.length + ' results)</h3>');
                ...
                if($('#activeSearch').text() == "Users"){
                    $('#search_results').empty();
                    $('#searchResultType').html('<h3>User results for "' + search_param + '" (' + response.data.length +'  results)</h3>');
                    $('#search_results').attr('class', 'userList col-sm-8 col-sm-offset-2');
                    for(i = 0; i < response.data.length; i++){
                        var tack_results_string = 
                        ...
                        '<button type="button" data-user=' + response.data[i].id + ' class="btn btn-primary right1 follow-user">Follow</button>' +
                        '</div>' +
                        '</div>' +
                        '</a>' +
                        '</div>';
